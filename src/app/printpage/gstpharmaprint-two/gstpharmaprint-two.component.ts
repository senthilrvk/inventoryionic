import { Component, OnInit , OnDestroy, AfterViewInit, ViewEncapsulation, Input, ChangeDetectorRef} from '@angular/core';
...
import { IssueSub } from 'src/app/models/IssueSub';
...
@Component({
  selector: 'app-gstpharmaprinttwo',
  templateUrl: './gstpharmaprint-two.component.html',

})
export class GstpharmaprintTwoComponent implements OnInit {
    @Input() BillNo: string;
    ...
    fngetPrint() {
      ...
      .pipe(takeUntil(this._unsubscribeAll))
        .toPromise().then(data => {
          ...
          this.totalQuantity = this.IssueSubDetailsInfo.map(data => data.IssueSub_Qty).reduce((prev, cur) => parseFloat(prev || 0) +  parseFloat(cur || 0));
          this.totalFreeQuantity = this.IssueSubDetailsInfo.map(data => data.IssueSub_FreeQty).reduce((prev, cur) => parseFloat(prev || 0) +  parseFloat(cur || 0));
