...
@Component({
  selector: 'app-orderinventory',
  templateUrl: './orderinventory.component.html',
  styleUrls: ['./orderinventory.component.scss'],

})

export class SaleinventoryComponent implements OnInit {
  @ViewChild('search', { static: false }) private searchFocus;

  staffId: any; BranchId: any; Apiurl: any; BillSeriesSelected: any; BillSeriesJson: any;
  Pricejson: any; billNo: any; todays: any = new Date(); PaytermsSelected: any; PriceSelected: any;
  Salesmanjson: any; SalesManSelectedId: any;
  bAdditionalCessInclusiveInSales = false;  shownNext: boolean;
  view_product: boolean; AcId: any = 0; Address: any; ProductData: any; productload: boolean;
  ProductAddcart = []; Softwarename: any; divshow: boolean; SettingBatch: any; SettingExpiry: any;
  NegativeBilling: any; displaycolumn = []; QtyDecPlace: any; product_show: boolean;
  CustomerForSoftware: any; AddOrMinus: any; MrpInclusiveSales: any; SRof: any; PackCal: any;
  RateDecimalPlace: any; OtherAmtTaxCalculation: any; EditDate: any;
  bSaveFlag: any; AcName: any; custPhone: any; customerGet: any; jsonTaxGet: any; UniqeuNo: any;
  showSearch: boolean; ListIssueSubDetailsInfo = [];
  ledgerAmount: number = 0;
  IssueSubDetailsInfo = {
    ItemDesc: '', Store_BatchSlNo: '', IssueSub_Batch: '', IssueSub_Pack: '',
    IssueSub_ExpDate: '', IssueSub_PurRate: '', IssueSub_PerRate: 0, IssueSub_OriginalRate: 0, IssueSub_Mrp: '', IssueSub_Qty: 0, IssueSub_FreeQty: 0,
    ...
    Agent_SubAmount: 0, IssueSub_SpRate1: '', IssueSub_SpRate2: '', IssueSub_SpRate3: '', IssueSub_SpRate4: '', IssueSub_SpRate5: '',
    IssueSub_RQty: 0, IssueSub_CessPers: 0, IssueSub_CessAmt: 0, BranchId: '', AvilableQTY: '',
  ...
  IssueInfo = {
    ...
    'AcId': 0, 'Issue_CustName': '', 'Remarks': '', 'Issue_PayTerms': 'CASH', 'Issue_SaleType': "0", 'Issue_AddCessFlag': false, BranchId: 0, StaffId: 0, DirectRBank: '',
    'Issue_TCSPers': 0, 'Issue_TCSAmt': 0,"Issue_GSTinNo": '', ListIssueSubDetailsInfo: [], DictionaryObject: {}, ListIssueTaxInfo: {}
  };
  cessValue = 'No'
  strCFWithTax: any; toogDisc = false;
  discount: number = 0;
  bCessInclusiveInSales = false;
  private _unsubscribeAll: Subject<any>;
  strTaxIncluded = 'No';
  ListIssueTaxInfo = [];
  bAgentCommisionCalcOnMarginPers = 'No';
  loading = false;
  dataGetList = [];
  productPop: boolean;
  bBillListShow: boolean;
  ...
  times: any;
  smsAlert: any;
   printFileName: any = 'GalaxyelectronicsComponent';
  bCustomerBillSeriesLinkInSales: boolean;
  bCompoundTaxInSales: boolean = false;
  ...
  async fnPrintTable() {
    // this.printFileName = 'PrintmodelOneComponent'
    let components: any;
  ...
  fnClear() {
    ...
    this.IssueInfo = {
      ...
      'Issue_OtherTaxPers': 0, 'Issue_CourierTaxPers': 0, 'BillSerId': 0, 'AgentPers': 0, 'Issue_VechileNo': '',
      ...
      , 'Issue_PayTerms': 'CREDIT', 'Issue_SaleType': "0", 'Issue_AddCessFlag': false,
      'Issue_TCSPers': 0, 'Issue_TCSAmt': 0, "Issue_GSTinNo": '',BranchId: 0, StaffId: 0, DirectRBank: this.times,
      ListIssueSubDetailsInfo: [], DictionaryObject: {}, ListIssueTaxInfo: {}
    };
  ...
  onAddcart(index, Rate, Qty, Free, disc) {
    ...
    if (!parseFloat(Qty || 0) && !parseFloat(Free || 0)) {
      this.alertToast('Enter Quantity', 'warning!')
      return
    }
  ...
  async alertToast(head, msg) {
    this.ctrlService.presentToast(head, msg);
  ...
  async stockCheckToast(stock) {
    ...
    //   header: 'Available Stock ' + product[0].Store_BalQty,
  ...
  fnProductGets(val) {
    ...
    this.salesService.fnProductGets('inventry', ProductSearch, this.AcId,
      this.BranchId, this.BillSeriesSelected,0, this.Apiurl)
      .pipe(takeUntil(this._unsubscribeAll))
      .toPromise().then(data => {
        ...
        // ProductId IssueSub_Qty IssueSub_FreeQty
        // ProductId StockQty

      }).catch((err) => this.productload = false);
  }
  ...
  fnProductKeyDown(nProductId) {
    ...
    this.salesService.onProductKeyDown(nProductId, this.BranchId,this.Apiurl)
    .pipe(takeUntil(this._unsubscribeAll))
      .subscribe(data => {
        const dataHistory = data;
        if (dataHistory.length == 0) {
          this.alertToast('Stock insufficient.', 'warning!')
          return;
        }
  ...
  async historyModal(product) {
    ...
    const myModal = await this.modalController.create({
      ...
      componentProps: {
        data: product, diffPrice: this.PriceSelected, ParamsGodownId: 0,
  ...
  async replaceOrAdd(item, rate, nqty, nFree,discPers, bstock, id, no) {
    ...
    const alert = await this.alertController.create({
      header: 'Already Exit!',
      message: `Quantity - ${oldQty} | Free -${oldFree}`,
  ...
  addNew(Rate, Qty, txtFree,discPers, nProductId, BatchNo) {
    ...
    this.salesService.addInventry('inventry',efreeQty, nProductId, this.BranchId, BatchNo,0, this.Apiurl)
    .pipe(takeUntil(this._unsubscribeAll))
    .toPromise()
      .then(res => {
        ...
        this.IssueSubDetailsInfo = {
          ...
          IssueSub_SGSTTaxPers: '', IssueSub_CGSTTaxPers: '', IssueSub_IGSTTaxPers: '', Field2: '', IssueSub_AddDisPers: 0, AgentPrice: 0,
          Agent_SubAmount: 0, IssueSub_SpRate1: '', IssueSub_SpRate2: '', IssueSub_SpRate3: '', IssueSub_SpRate4: '', IssueSub_SpRate5: '',
          IssueSub_RQty: 0, IssueSub_CessPers: 0, IssueSub_CessAmt: 0, BranchId: '', AvilableQTY: '', IssueSub_DistRate: 0,
          IssueSub_ExtraCessPers: 0, IssueSub_ExtraCessAmt: 0, IssueSub_NoField1: 0,
        };
        ...
        // if (parseFloat(dataget.   || 0) > 0) {
        //   this.IssueSubDetailsInfo['IssueSub_PurRate'] = dataget.Store_BarCode;
        // } else {
        //   this.IssueSubDetailsInfo['Store_ReceiptRate'] = dataget.Store_BarCode;
        // }

        //   if (this.MrpInclusiveSales == "Yes") {
        //     dMarginSelRate = dMarginSelRate - ((dMarginSelRate * dMarginTaxPers) / (100 + dMarginTaxPers));
        // }
        ...
        //   this.ListIssueSubDetailsInfo.forEach(data => {

        //     const idx = dataget.ProductId;
        //     const idy = parseFloat(data.ProductId) || 0;
        //     if (idx === idy && dataget.Store_BatchSlNo === data.Store_BatchSlNo) {
      ...
      if (oIssueSubDetailsInfoArg.ProductId !== 0) {
        ...
        if (oIssueSubDetailsInfoArg.IssueSub_Repl === 'Rep' && this.Softwarename === 'WholeSalePharma') {
          ...
          if (strMrpInclusiveSales === 'Yes') {
            ...
            if (this.bCessInclusiveInSales && strSalesType != "INTERSTATE") {
              dAgentPrice = dAgentPrice - ((dAgentPrice * dCessPers) / (100 + dCessPers));
              oIssueSubDetailsInfoArg.AgentPrice = dRate;
            }
            if (this.bCessInclusiveInSales === true && this.IssueInfo.Issue_AddCessFlag && strType !== 'INTERSTATE') {
              if (this.bAdditionalCessInclusiveInSales) {
          ...
          if (strMrpInclusiveSales === 'Yes') {
            ...
            if (this.bCessInclusiveInSales && strType !== 'INTERSTATE') {
              dAgentPrice = dAgentPrice - ((dAgentPrice * dCessPers) / (100 + dCessPers));
              oIssueSubDetailsInfoArg.AgentPrice = dRate;
            }
          ...
          if (this.IssueInfo.Issue_AddCessFlag && strType !== 'INTERSTATE') {
  ...
  fnGetFinalTotal() {
    ...
    dOtherCharge = this.IssueInfo.Issue_OtherCharge;
  ...
  async fnsave() {
    ...
    this.disSave = true;
    this.appservice.fnApiPost(this.Apiurl + '/Sales/fnSave', body)
    .pipe(takeUntil(this._unsubscribeAll))
      .subscribe(data => {
        const jsonobj = data;
      ...
      .then(data => {
        ...
        objIssueInfo.forEach(IssueMain => {
          this.IssueInfo = {
            ...
            'Issue_Freight': IssueMain.Issue_Freight, Issue_VechileNo: IssueMain.Issue_VechileNo,
            ...
            'AcId': IssueMain.AcId, 'Issue_CustName': IssueMain.Issue_CustName, 'Remarks': IssueMain.Remarks, 'Issue_PayTerms': IssueMain.Issue_PayTerms
            , 'Issue_SaleType': String(IssueMain.Issue_SaleType), 'Issue_AddCessFlag': IssueMain.Issue_AddCessFlag,
            BranchId: IssueMain.BranchId, StaffId: IssueMain.StaffId,"Issue_GSTinNo": IssueMain.Tin1, 'Issue_TCSPers': IssueMain.Issue_TCSPers, 'Issue_TCSAmt': IssueMain.Issue_TCSAmt,
            DirectRBank: IssueMain.DirectRBank,
            ListIssueSubDetailsInfo: IssueMain.ListIssueSubDetailsInfo, DictionaryObject: IssueMain.DictionaryObject, ListIssueTaxInfo: IssueMain.ListIssueTaxInfo
          };
        ...
        objIssueSubDetailsInfo.forEach(IssueSub => {
          ...
          this.IssueSubDetailsInfo.IssueSub_OriginalRate = Number(IssueSub.IssueSub_OriginalRate);
  ...
  fnGrossValueCalculation(data) {
    let dGrossValue, dQty, dRate = 0;
     dQty = parseFloat(data.IssueSub_Qty || 0);
     dRate = parseFloat(data.IssueSub_OriginalRate || 0);
  ...
  fnShare() {
    let data = [{ 'name': 'John', 'age': 31, 'city': 'New York' }, { 'name': 'John', 'age': 31, 'city': 'New York' }]
    const csv: any = this.convertToCSV(data);
    const fileName: any = 'team.txt';
